...
def estimate_victory_probability(num_episodes=100000):
	...
	fight_counts = np.zeros(len(env.guards), dtype=int)
	win_counts   = np.zeros(len(env.guards), dtype=int)
	...
	for ep in range(num_episodes):
		...
		while not done:
			...
			if result_text.startswith('Fought'):
				# parses for gaurd name
				guard = result_text.split()[1]
				idx   = int(guard[1]) - 1
				...
				# tracks if won
				won = ('won' in result_text) or (reward > 0)
				if won:
					win_counts[idx] += 1
				
				# dubugging print
				#print(f"[Episode {ep:3d}] fought {guard} → "
                      #f"win={won}  "
                      #f"counts=(fights {fight_counts[idx]}, wins {win_counts[idx]})")
	
	# gets the probabailities
	for i in range(len(P)):
		if fight_counts[i] > 0:
			P[i] = win_counts[i] / fight_counts[i]
...
if __name__ == '__main__':
    # make runs deterministic for debugging
    random.seed(0)
    np.random.seed(0)

    # run only 100 episodes so you can read the output easily
    P = estimate_victory_probability(num_episodes=10)
    print("\nFinal estimated victory probabilities:")
